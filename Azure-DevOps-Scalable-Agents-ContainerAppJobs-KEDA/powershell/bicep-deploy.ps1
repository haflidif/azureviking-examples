param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$adoResourceGroupName = "cnappjobs-ado-keda-rg",
    ...
    [Parameter(Mandatory = $false)]
    [string]$adoUserAssignedIdentityName = "cnappjobs-ado-keda-mi",
    ...
    [Parameter(Mandatory = $true)]
    [string]$adoPoolName = "cnappjobs-ado-keda-pool",

    [Parameter(Mandatory = $false)]
    [string]$resourcePrefix = "cnappjobs-ado-keda"
        
)

$preReqAttributes = @{
    resourcePrefix              = $resourcePrefix
    resourceGroupName           = $adoResourceGroupName
    userAssignedIdentityName    = $adoUserAssignedIdentityName
...
if ($runOnlyPreReq) {
    ...
    $PreReqAdo = . "$PSScriptRoot/pre-req.ps1" @preReqAttributes
    # $PreReqAdo = .\pre-req.ps1 @preReqAttributes
...
if (!$runOnlyPreReq -and !$cleanup) {
    ...
    # $PreReqTF | ConvertTo-Json -Depth 100
    # Write-Progress -Activity "Deploying pre-requisites for the Terraform Infrastructure" -Id 1 -Status "Completed" -Completed
    Write-Progress -Activity "Deploying pre-requisites for the AzureDevOps Self-Hosted Agent Infrastructure" -Id 1 -Status "Completed" -Completed


    $adoBicepFilePath = "$PSScriptRoot/../bicep"
    ...
    $adoDeploymentParameters = @{
        ...
        'azdoOrganizationName'     = $PreReqAdo.AzureDevOpsPool.OrganizationName
        'poolName'                 = $PreReqAdo.AzureDevOpsPool.Name
